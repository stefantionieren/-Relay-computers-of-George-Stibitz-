...
貝爾的工程師們設計了計算機電路,這些電路在計算的每一個步驟上都檢查自己。 電路的設計不僅是爲了增加,減少,儲存號碼等等; 還是爲了檢查它們是否正確地做了這些事情,並且爲了不使用機器而設計。 貝爾的工程師們還根據他們在設計電話線路方面的經驗進行指導,這些線路在常常充滿敵意的環境中不得不長時間工作。 這些電路由半熟練的技術人員設計,如果每次電話線路中斷或客戶電話不通時,必須叫一個工程師來修理,電話服務將付出巨大的代價。 Bell Labs Model III 通過VI使用的是一個系統,在這個系統中,每個小數點編碼不是四個而是七個二進制繼電器。 它們被分成兩組和五組,十進制編碼如下:
